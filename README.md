...
- 開発の過程をQiitaに投稿していたこと。
    ...
    - **実装して終わり**ではなく、ドキュメント化することで**記憶ではなく記録に残る**ります。このようにドキュメンテーション力を活かして知識を積み上げていき、知識の点と点を繋げられることが私の強みです。
    - 誰かに教えることを想定して記事を書くことで、自分の言葉で噛み砕く必要があり、その結果より深く理解することができます。
...
- CircleCIを導入してCI/CDパイプラインを構築
    ...
    - CircleCIが実行されるとcapistranoが走り自動デプロイされることで、デプロイコマンドを忘れるというヒューマンエラーを仕組み化で解決しました。こういった**人の作業が自動化されることにプログラミングの面白さや可能性を感じます。**
...
- 管理者機能を導入して本番運用を意識
    - 実際に運用した時に不適切な投稿をするユーザーがいることを考え、投稿を削除できる権限を持った管理者ユーザー機能を追加
- タグ入力時に自動補完機能を導入
    - UXの向上を図るために、必要な機能を考えた結果、多くのアプリで導入されている自動補完を実装することを決めて開発
- 徹底的なリファクタリング
    - DRYの原則を考え、共通部分のほぼ全てをテンプレート化することで、仕様変更に対して柔軟に対応可能
- テストコード数が100件以上
    - 目的は、テストコードをしっかりと書くことでバグを未然に防ぎアプリの品質を向上する
- 静的解析ツールRubocopを導入してコードの品質を担保
    - 個人で開発していると、「自分が読めれば良いコード」が増えてしまうと思いました。
    - 規約に沿ってコードを書くことで、誰が読んでも分かるコードになって保守性が上がることで結果的に開発効率が向上する。と考えて、導入しました。
- 実際にアプリを使ってもらい、フィードバックを受けて改善
    - 友人にアプリを使ってもらい、改善点を抽出し即座に反映してアプリをより良いものにするために考えて行動しています。
- 主要機能を非同期通信で実装
